...
const MovieDetails = () => {
  ...
  const handleFavourite = async () => {
    try {
      if (!user) {
        return toast.error("Please login to add favourites");
      }
    ...
    <div className="px-6 md:px-16 lg:px-40 pt-30 md:pt-50">
      <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
        <img
          ...
          alt=""
          className="max-md:mx-auto rounded-xl h-140 max-w-70 object-cover"
        />
        <div className="relative flex flex-col gap-3">
          ...
          <div className="flex items-center flex-wrap gap-4 mt-4">
            <button className="flex items-center gap-2 px-7 py-3 text-sm bg-gray-800 hover:bg-gray-900 transition rounded-md font-medirm cursor-pointer active:scale-95">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {shows.slice(1, 5).map((movie, index) => (
